...
always have the following structure:
  ...
  <Karfunkel> #As always. This is the root.
    ...
    <response type="OpenProject" id="12" status="finished"> #The request type you wanted Karfunkel to process and the command ID you sent along with your request. A list of possible stati can be found below.
...
map's name if it has been renamed by another client).
...
want to debug Karfunkel <b>do not use</b> this request.
  ...
  <request type="Eval" id="YOUR_REQ_ID">
...
No exception:
  ...
  <response type="Eval" id="YOUR_REQ_ID" status="ok">
...
Additionally, a warning is logged on the server side.
  ...
  <response type="Eval" id="YOUR_REQ_ID" status="rejected">
...
=====OK:
    ...
    <response type="Hello" id="0" status="ok"> #The request ID is still 0
...
=====Rejected:
    ...
    <response type="Hello" id="0" status="rejected">
...
server to obtain the name of a project file.
...
This is sent when Karfunkel accepted your request and now loads the project.

  <response type="OpenProject" id="X" status="processing" />

===== Finished

Send when the project is in a usable state.

    <response type="OpenProject" id="X" status="finished" />

=====Rejected:

  <response type="OpenProject" id="X" status="rejected">
    <reason>REASON_WHY_YOU_WERE_REJECTED</reason>
  </response>
...
can send one to Karfunkel (or another client) as well.
  ...
  <request type="Ping" id="X" />

==== Possible responses

  <response type="Ping" id="X" status="ok" />

Tells the requestor that you're available.
...
Instructs Karfunkel to load the project with the given name.
    ...
    <request type="SelectProject" id="X">
      <name>project_name</name> #Without the .rmk extension
    </request>
  
==== Possible responses
  
  [OK]
    The Project was selected. You can work with it now.
  [failed]
    The project couldn't be select for some reason. Check the
    +reason+ tag.
...
[name] Name of the project loaded.