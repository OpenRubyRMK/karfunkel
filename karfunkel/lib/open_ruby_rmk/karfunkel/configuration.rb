  ...
  class Karfunkel
    ...
    class Configuration
      ...
      def check!
        ...
        @config.keys.each do |option|
          unless defaults.has_key?(option)
            raise(Errors::ConfigurationError, "Unknown configuration option '#{option}'.")
          end
        end
      end
      ...
      pluggify do
        ...
        #values they have prior to any setting.
      ...
      def method_missing(sym, *args, &block)
        if @config.has_key?(sym)
          ...
          raise(Errors::ConfigurationError, "Unknown directive/option '#{sym}'!")
        end
      end
